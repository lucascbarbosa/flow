...
\subsection{Milestone 1}
O modelo NeuralODE foi treinado no dataset 2D e os resultados demonstram a capacidade do modelo em aprender transformações contínuas e invertíveis entre a distribuição dos dados e uma distribuição Gaussiana. O setup do experimento envolve a criação do dataset, modelo e otimizador, como descrito no Código~\ref{code:milestone1_setup}:
\begin{lstlisting}[language=Python, caption=Setup do experimento Milestone 1, label=code:milestone1_setup]
# Dataset
dataset = Synthetic2D(
    n_samples=5000,
    noise=0.05,
...
\begin{figure}[hbt!]
    \centering
    \includegraphics[width=0.5\textwidth]{imgs/01_moons_neuralode_trajectories.png}
    \includegraphics[width=0.5\textwidth]{imgs/01_circles_neuralode_trajectories.png}
    \includegraphics[width=0.5\textwidth]{imgs/01_spirals_neuralode_trajectories.png}
...
\begin{figure}[hbt!]
    \centering
    \includegraphics[width=0.5\textwidth]{imgs/01_moons_neuralode_vector_fields.png}
    \includegraphics[width=0.5\textwidth]{imgs/01_circles_neuralode_vector_fields.png}
    \includegraphics[width=0.5\textwidth]{imgs/01_spirals_neuralode_vector_fields.png}
...
\subsubsection{Tranformação: $z \sim N(0,I) \to x = \phi(z,1)$}
...
\begin{figure}[hbt!]
    \centering
    \includegraphics[width=0.5\textwidth]{imgs/01_moons_neuralode_transformation.png}
    \includegraphics[width=0.5\textwidth]{imgs/01_circles_neuralode_transformation.png}
    \includegraphics[width=0.5\textwidth]{imgs/01_spirals_neuralode_transformation.png}
...
\newpage
\subsubsection{Experimento 1: Análise de NFEs}
A Tabela \ref{tab:milestone1_exp} descreve as diferentes configurações de solvers e tolerâncias usadas e suas respectivas métricas:
...
\begin{table}[hbt!]
    ...
    \begin{tabular}{|cccc|ccc|}
        ...
        \hline
    \end{tabular}
    \caption{Experimento 1: Configurações entre solvers e tolerâncias.}
    \label{tab:milestone1_exp}
\end{table}

\subsection{Milestone 2}
O modelo CNF foi treinado em dois datasets: dataset 2D sintético e MNIST reduzido (100 dimensões após PCA). Os resultados demonstram a capacidade do modelo em aprender transformações contínuas e invertíveis entre a distribuição dos dados e uma distribuição Gaussiana base. Os setups dos experimentos envolvem a criação dos datasets, modelos e otimizadores, como descrito nos Códigos~\ref{code:milestone2_setup_2d} e~\ref{code:milestone2_setup_mnist}:

\begin{lstlisting}[language=Python, caption=Setup do experimento Milestone 2 - Dataset 2D, label=code:milestone2_setup_2d]
# Dataset 2D
dataset_2d = Synthetic2D(
    n_samples=5000,
    noise=0.05,
    dataset_type='moons'
)
...
\begin{lstlisting}[language=Python, caption=Setup do experimento Milestone 2 - MNIST Reduzido, label=code:milestone2_setup_mnist]
# Dataset MNIST Reduzido
...
\subsubsection{Trajetórias Contínuas: $x \to z$}
Para o dataset 2D sintético, as trajetórias são visualizadas diretamente no espaço 2D. A Figura~\ref{fig:milestone2_trajectories_2d} ilustra as trajetórias contínuas aprendidas pelo modelo CNF no dataset \texttt{moons}. O modelo aprende a transformar os dados em direção a uma distribuição Gaussiana, mantendo a estrutura dos dados enquanto realiza a transformação suave.

Para o dataset MNIST reduzido, as trajetórias são visualizadas através de projeção PCA para 2D. A Figura~\ref{fig:milestone2_trajectories_mnist} ilustra as trajetórias contínuas aprendidas pelo modelo CNF. Cada ponto representa uma amostra projetada no espaço 2D das duas primeiras componentes principais. O modelo aprende a transformar os dados de MNIST (projetados) em direção a uma distribuição Gaussiana, mantendo a estrutura dos dígitos enquanto realiza a transformação suave.

\begin{figure}[hbt!]
    \centering
    \includegraphics[width=0.8\textwidth]{imgs/02_moons_cnf_transformation.png}
    \caption{Trajetórias contínuas aprendidas pelo CNF para o dataset 2D \texttt{moons}. A figura mostra a transformação de $z \sim N(0,I)$ (esquerda) para $x$ (direita).}
    \label{fig:milestone2_trajectories_2d}
\end{figure}

\begin{figure}[hbt!]
    \centering
    \includegraphics[width=0.8\textwidth]{imgs/02_moons_cnf_transformation.png}
    \caption{Trajetórias contínuas aprendidas pelo CNF para MNIST reduzido, visualizadas através de projeção PCA 2D. A figura mostra a transformação de $z \sim N(0,I)$ (esquerda) para $x$ (direita).}
    \label{fig:milestone2_trajectories_mnist}
\end{figure}

\subsubsection{Campo Vetorial Aprendido: $\dot{x} =f(x,t)$}
A Figura~\ref{fig:milestone2_vectorfield} mostra o campo vetorial $f(x, t)$ aprendido pelo modelo CNF em um grid 2D para o dataset \texttt{moons}, para diferentes valores de $t \in [0, 0.5, 1]$. As setas indicam a direção e magnitude do campo vetorial em cada ponto do espaço. O campo vetorial aprende a direcionar os pontos em direção à distribuição Gaussiana, com maior magnitude nas regiões onde há maior densidade de dados.

\begin{figure}[hbt!]
    \centering
    \includegraphics[width=0.8\textwidth]{imgs/02_moons_cnf_vector_fields.png}
    \caption{Campo vetorial $f(x, t)$ aprendido pelo CNF em um grid 2D para o dataset \texttt{moons}, para diferentes valores de tempo $t$.}
    \label{fig:milestone2_vectorfield}
\end{figure}

\subsubsection{Transformação: $z \sim N(0,I) \to x = \phi(z,1)$}
A Figura~\ref{fig:milestone2_transformation_2d} apresenta a transformação aprendida pelo modelo CNF no dataset 2D. Observa-se que o modelo consegue mapear efetivamente amostras da distribuição Gaussiana base para a distribuição dos dados, demonstrando a capacidade do CNF em modelar distribuições complexas através de transformações contínuas e invertíveis.

Para o dataset MNIST reduzido, a Figura~\ref{fig:milestone2_transformation_mnist} apresenta a transformação aprendida pelo modelo CNF. O modelo consegue mapear efetivamente amostras da distribuição Gaussiana base para a distribuição dos dados de MNIST (projetados em 2D), demonstrando a escalabilidade do CNF para dimensões intermediárias.

\begin{figure}[hbt!]
    \centering
    \includegraphics[width=0.8\textwidth]{imgs/02_moons_cnf_transformation.png}
    \caption{Transformação aprendida pelo CNF de $z \sim N(0,I)$ (esquerda) para $x$ (direita) para o dataset 2D \texttt{moons}.}
    \label{fig:milestone2_transformation_2d}
\end{figure}

\begin{figure}[hbt!]
    \centering
    \includegraphics[width=0.8\textwidth]{imgs/02_moons_cnf_transformation.png}
    \caption{Transformação aprendida pelo CNF de $z \sim N(0,I)$ (esquerda) para $x$ (direita) para MNIST reduzido, visualizada através de projeção PCA 2D.}
    \label{fig:milestone2_transformation_mnist}
\end{figure}

\subsection{Milestone 3}
O modelo FFJORD foi treinado no dataset MNIST completo (784 dimensões) utilizando o estimador Hutchinson para escalabilidade. Os resultados demonstram a capacidade do modelo em aprender transformações contínuas em alta dimensionalidade. O setup do experimento envolve a criação do dataset com preprocessing adequado, modelo e otimizador, como descrito no Código~\ref{code:milestone3_setup}:

\begin{lstlisting}[language=Python, caption=Setup do experimento Milestone 3, label=code:milestone3_setup]
# Dataset com preprocessing (dequantização + logit transform)
...
\subsubsection{Trajetórias Contínuas: $x \to z$}
Para o dataset MNIST completo, as trajetórias são visualizadas através de projeção PCA para 2D após a transformação. A Figura~\ref{fig:milestone3_trajectories} ilustra as trajetórias contínuas aprendidas pelo modelo FFJORD. O modelo demonstra capacidade de transformar dados de alta dimensionalidade (784D) em direção a uma distribuição Gaussiana, utilizando o estimador Hutchinson para viabilizar o treinamento.

\begin{figure}[hbt!]
    \centering
    \includegraphics[width=0.8\textwidth]{imgs/03_mnist_ffjord_transformation.png}
    \caption{Trajetórias contínuas aprendidas pelo FFJORD para MNIST completo, visualizadas através de projeção PCA 2D. A figura mostra a transformação de $z \sim N(0,I)$ (esquerda) para $x$ (direita).}
    \label{fig:milestone3_trajectories}
\end{figure}

\subsubsection{Campo Vetorial Aprendido: $\dot{x} =f(x,t)$}
A Figura~\ref{fig:milestone3_vectorfield} mostra o campo vetorial $f(x, t)$ aprendido pelo modelo FFJORD em um grid 2D (projetado via PCA) para diferentes valores de $t$. O campo vetorial utiliza uma arquitetura CNN para processar imagens de 28×28 pixels, permitindo capturar padrões espaciais locais dos dígitos. A regularização de energia cinética ajuda a suavizar o campo vetorial, melhorando a estabilidade numérica da integração ODE.

\begin{figure}[hbt!]
    \centering
    \includegraphics[width=0.8\textwidth]{imgs/03_mnist_ffjord_vector_fields.png}
    \caption{Campo vetorial $f(x, t)$ aprendido pelo FFJORD em um grid 2D (projetado via PCA) para diferentes valores de tempo $t$.}
    \label{fig:milestone3_vectorfield}
\end{figure}

\subsubsection{Transformação: $z \sim N(0,I) \to x = \phi(z,1)$}
A Figura~\ref{fig:milestone3_transformation} apresenta a transformação aprendida pelo modelo FFJORD. O modelo consegue gerar amostras realistas de dígitos MNIST a partir de amostras da distribuição Gaussiana base, demonstrando a eficácia do estimador Hutchinson para escalar CNFs a dimensões altas. As amostras geradas preservam características visuais dos dígitos originais, indicando que o modelo aprendeu uma transformação efetiva mesmo em 784 dimensões.

\begin{figure}[hbt!]
    \centering
    \includegraphics[width=0.8\textwidth]{imgs/03_mnist_ffjord_transformation.png}
    \caption{Transformação aprendida pelo FFJORD de $z \sim N(0,I)$ (esquerda) para $x$ (direita) para MNIST completo, visualizada através de projeção PCA 2D e reconstrução de imagens.}
    \label{fig:milestone3_transformation}
\end{figure}
